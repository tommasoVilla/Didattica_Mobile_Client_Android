...
/*
 * An ExamDaoFactory object is responsible to build an object that implements ExamDao interface.
...
public class ExamDaoFactory {
    private static ExamDaoFactory instance;

    private ExamDaoFactory() {}

    public static synchronized ExamDaoFactory getInstance() {
        if (instance == null) {
            instance = new ExamDaoFactory();
        }
        return instance;
    }

    public ExamDao createExamDao() throws InvalidConfigurationException {
        try {
            String className = AppConfiguration.getInstance().readProperty("examdao_type");
            Class<?> c = Class.forName(className);
            return (ExamDao) c.newInstance();
