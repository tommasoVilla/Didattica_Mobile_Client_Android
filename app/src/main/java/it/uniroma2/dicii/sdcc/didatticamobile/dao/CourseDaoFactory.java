package it.uniroma2.dicii.sdcc.didatticamobile.dao;

import java.io.IOException;

import it.uniroma2.dicii.sdcc.didatticamobile.config.AppConfiguration;
import it.uniroma2.dicii.sdcc.didatticamobile.error.InvalidConfigurationException;

/*
 * An CourseDaoFactory object is responsible to build an object that implements CourseDao interface.
 * The concrete class is read from the configuration parameters stored in AppConfiguration.
 * The class is implemented as a singleton.
 * */
public class CourseDaoFactory {

    private static CourseDaoFactory instance;

    private CourseDaoFactory() {
    }

    public static synchronized CourseDaoFactory getInstance() {
        if (instance == null) {
            instance = new CourseDaoFactory();
        }
        return instance;
    }

    public CourseDao createCourseDao() throws InvalidConfigurationException {
        try {
            String className = AppConfiguration.getInstance().readProperty("coursedao_type");
            Class<?> c = Class.forName(className);
            return (CourseDao) c.newInstance();
        } catch (NullPointerException | IOException | ClassNotFoundException | InstantiationException | IllegalAccessException e) {
            throw new InvalidConfigurationException(e.getMessage(), e.getCause());
        }
    }
}
